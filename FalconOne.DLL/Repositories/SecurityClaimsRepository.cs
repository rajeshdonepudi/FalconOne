...
namespace FalconOne.DAL.Repositories
{
    public class SecurityClaimsRepository : GenericRepository<SecurityClaim>, ISecurityClaimsRepository
    {
        ...
        {
            return await _context.SecurityClaims.FirstOrDefaultAsync(x => x.Id == claimId && x.TenantId == tenantId, cancellationToken);
        }
        ...
        {
            return await _context.SecurityClaims.Where(x => x.ApplicationPolicyId == policyId).ToListAsync(cancellationToken);
        }
        ...
        {
            return await _context.SecurityClaims.Where(x => x.TenantId == tenantId).Select(x => new KeyValuePair<Guid, string>(x.Id, x.Value)).ToListAsync(cancellationToken);
        }
    }
}
